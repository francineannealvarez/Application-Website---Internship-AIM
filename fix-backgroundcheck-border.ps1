# ============================================
# FIX: BackgroundCheckContent.tsx syntax error (same cut-off border edit issue)
# Restored T.border, fixed 2 broken style lines, added consistent borders
# throughout (select input, upload dropzone, preview box, warning box)
# ============================================

$targetPath = "src\components\dashboard\BackgroundCheckContent.tsx"
...
const selectStyle: React.CSSProperties = { backgroundColor: '#F7F9FA', borderColor: T.border, color: '#0B2A4A' };
...
export default function BackgroundCheckContent({ isCurrent, onSubmit }: { isCurrent: boolean; onSubmit: () => void }) {
  ...
  return (
    <div className="pt-3 space-y-4">
      ...
      <div className="rounded-xl p-5 space-y-4 text-sm leading-relaxed text-justify" style={{ backgroundColor: T.bg, border: `1px solid ${T.border}`, color: T.navy }}>
        <p>
      ...
      <div>
        ...
        {signaturePreview ? (
          <div className="flex items-center gap-3 p-3 rounded-xl" style={{ backgroundColor: T.bg, border: `1px solid ${T.border}` }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={signaturePreview} alt="E-signature preview" className="h-16 object-contain bg-white rounded" style={{ border: `1px solid ${T.border}` }} />
...
Write-Host ""
Write-Host "Done! Naayos na ang syntax error sa BackgroundCheckContent.tsx. Consistent na ang borders sa buong file." -ForegroundColor Cyan
