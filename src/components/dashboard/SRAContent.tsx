...
export default function SRAContent({ isCurrent, onSubmit, applicationId }: { isCurrent: boolean; onSubmit: () => void; applicationId: string | null }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    fetch(`/api/sra?application_id=${encodeURIComponent(applicationId)}`)
      .then((res) => (res.ok ? res.json() : null))
      .then((data: { status?: string } | null) => {
        if (!active) return;
        if (data?.status === 'Submitted') setSubmitted(true);
      })
      .catch((err) => console.error('Load SRA error:', err))
      .finally(() => {
    ...
    return (
      <div className="pt-3">
        <div className="flex items-center gap-2.5 rounded-xl p-4 text-sm" style={{ backgroundColor: T.bg, color: T.gray }}>
          ...
          <span>Checking your SRA submission...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="pt-3">
        <div className="flex items-start gap-2.5 rounded-xl p-4 text-sm" style={{ backgroundColor: T.cyanBg }}>
          ...
          <span style={{ color: T.navy }}>
            Your SRA (Verbal Reasoning Test) answer sheet has been submitted. HR is reviewing your results, so please wait for an update before the next stage unlocks.
          </span>
        </div>
      </div>
    );
      ...
      <div className="rounded-xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-4" style={{ backgroundColor: T.bg }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-sm" style={{ color: T.navy }}>SRA - Verbal Reasoning Test</div>
